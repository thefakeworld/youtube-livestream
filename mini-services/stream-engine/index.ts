...
const YTDL_PATH = "/home/z/.local/bin/yt-dlp";
const FALLBACK_VIDEO_PATH = "/home/z/my-project/download/standby/fallback.mp4";
...
function getDiskStats(): { used: number; total: number } {
  try {
    const output = execSync("df -h --output=used,size /home/z/my-project 2>/dev/null || df -h --output=used,size / 2>/dev/null", {
      encoding: "utf-8",
    });
...
class PlaylistRunner {
  ...
  private resolveVideoPath(video: PlaylistVideo): string | null {
    ...
    if (video.youtubeId) {
      const cached = `/home/z/my-project/download/videos/${video.youtubeId}.mp4`;
  ...
  private downloadVideo(video: PlaylistVideo): string | null {
    ...
    const outputPath = `/home/z/my-project/download/videos/${filename}.mp4`;
    const downloadDir = `/home/z/my-project/download/videos`;
    ...
    console.log(`[playlist]   Command: ${YTDL_PATH} ${args.join(" ")}`);

    // Use spawnSync to capture full stdout/stderr with extended timeout (5 min)
    try {
      ...
      console.log(`[playlist] Task ${this.taskId}: yt-dlp exited with code ${result.status}`);
      console.log(`[playlist]   Signal: ${result.signal || 'none'}`);
      ...
      if (result.error) {
        console.error(`[playlist] Task ${this.taskId}: Spawn error: ${result.error.message}`);
        if (result.error.message.includes('ETIMEOUT') || result.error.message.includes('timed out')) {
      ...
      if (result.status !== 0) {
        ...
        console.error(`[playlist]   Exit code: ${result.status}, Signal: ${result.signal || 'none'}`);
      ...
      console.error(`[playlist] Task ${this.taskId}: ❌ Download EXCEPTION for video ${video.id}: ${errMsg}`);
      return null;
    }
  }
...
class ProcessManager {
  ...
  async startRelay(req: RelayStartRequest): Promise<{ success: boolean; pid?: number; taskId: string; error?: string }> {
    ...
    // Step 1: Get the real YouTube stream URL via yt-dlp
    let streamUrl: string;
    try {
      const quality = req.sourceQuality || "best";
      const ytdlpArgs = [
        "--js-runtimes", "node",
        "-f", quality,
        "--print", "url",
      ...
      console.log(`[relay] Fetching stream URL: yt-dlp ${ytdlpArgs.join(" ")}`);

      streamUrl = execSync(`"${YTDL_PATH}" ${ytdlpArgs.join(" ")}`, {
        encoding: "utf-8",
        timeout: 30000,
      }).trim();
...
async function ensureFallbackVideo(): Promise<void> {
  ...
  const dir = "/home/z/my-project/download/standby";
  if (!existsSync(dir)) {

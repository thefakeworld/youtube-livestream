...
import { appendFileSync, readdirSync, unlinkSync, existsSync, mkdirSync } from "fs";
...
class ProcessManagerSingleton {
  ...
  constructor() {
    this.videoDir = process.env.DOWNLOAD_DIR || "/home/z/my-project/download/videos";
    this.logDir = process.env.LOG_DIR || "/home/z/my-project/download/logs";
    this.ytDlpPath = process.env.YT_DLP_PATH || "/home/z/.local/bin/yt-dlp";
    this.ffmpegPath = process.env.FFMPEG_PATH || "/usr/bin/ffmpeg";
    this.cookiesPath = process.env.COOKIES_PATH || "/home/z/my-project/download/cookies.txt";
  ...
  startDownload(opts: {
    ...
    const args = [
      "--no-playlist",
      "--js-runtimes", "node",
      "-f", quality || "best[height<=1080][ext=mp4]/best[height<=720]/best",
      "-o", join(this.videoDir, "%(id)s.%(ext)s"),
      "--write-info-json",
      "--newline",
      "--progress",
      "--no-warnings",
    ...
    console.log(`[ProcessManager]   Command: ${this.ytDlpPath} ${args.join(' ')}`);

    return this._spawnYtDlp(taskId, args);
  }
  ...
  private _spawnYtDlp(taskId: string, args: string[]): ManagedProcess {
    ...
    // 解析 yt-dlp 进度输出
    // 格式: [download]  45.2% of ~25.63MiB at 3.45MiB/s ETA 00:06

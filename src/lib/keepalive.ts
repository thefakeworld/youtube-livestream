...
import { KEEPALIVE_LOG_DIR, CADDY_BASE_URL, NEXT_PORT } from '@/lib/paths';
...
const STATUS_FILE = join(KEEPALIVE_LOG_DIR, 'keepalive-status.json');

const TARGETS = [
  { name: 'caddy-gateway', url: CADDY_BASE_URL },
  { name: 'api-monitor',   url: `${NEXT_BASE_URL}/api/monitor` },
  { name: 'api-processes', url: `${NEXT_BASE_URL}/api/processes` },
];
...
function ensureLogDir() {
  if (!existsSync(KEEPALIVE_LOG_DIR)) {
    mkdirSync(KEEPALIVE_LOG_DIR, { recursive: true });
  }
}

...
# ── 检查并安装 yt-dlp（沙箱重建后可能丢失） ──
log_step_start "Checking yt-dlp"
YTDLP_PATH="$PROJECT_DIR/download/tools/yt-dlp"
if [ ! -f "$YTDLP_PATH" ] || ! "$YTDLP_PATH" --version >/dev/null 2>&1; then
        echo "[TOOLS] yt-dlp not found or broken, installing..."
        mkdir -p "$PROJECT_DIR/download/tools"
        curl -L https://github.com/yt-dlp/yt-dlp/releases/latest/download/yt-dlp -o "$YTDLP_PATH"
        chmod +x "$YTDLP_PATH"
        echo "[TOOLS] yt-dlp installed: $($YTDLP_PATH --version)"
else
        echo "[TOOLS] yt-dlp OK: $($YTDLP_PATH --version)"
fi
log_step_end "Checking yt-dlp"

# ── 清理残留进程（排除 3000，系统会自动启动 bun run dev） ──
log_step_start "Cleaning stale processes"
...
echo "[CLEAN] Mini-service ports clear"
log_step_end "Cleaning stale processes"

# ── 启动或复用 Next.js dev server ──
log_step_start "Starting Next.js dev server"
echo "[BUN] Checking if Next.js dev server is already running..."
DEV_PID=""

# 检查 3000 端口是否已被系统启动的 dev server 占用
existing_pid=$(lsof -t -i :3000 2>/dev/null || true)
if [ -n "$existing_pid" ]; then
        echo "[BUN] Port 3000 already in use by PID $existing_pid, reusing system dev server"
...
# ── 前台阻塞，保持脚本（和所有子进程）存活 ──
echo ""
echo "🎉 All services running. This script will stay alive to keep processes running."
echo "   3000: Next.js dev server"
echo "   3001: Stream Engine"
echo "   4000: Proxy -> 3000"
